            ...
            if self.predecessor != None:
                try:
                    with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
                        ...
                        s.sendall(CHECK_PREDECESSOR.encode('utf-8'))
                        # guardamos la info recibida
                        self.repli_pred = s.recv(1024).decode()
                    ...
                    self.broadcast.update_finger(
                        self.predecessor.id, ip_pred_pred if self.first else self.ip, TCP_PORT)
                    ...
                    if ip_pred_pred != self.ip:
                        try:
                            ...
                            with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
                                ...
                                s.settimeout(5)
                                s.sendall(
                                    f'{FALL_SUCC}|{self.ip}|{self.port_tcp}')
                                s.recv(1024).decode()
...
# region Sockets
        ...
        with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
            ...
            while True:
                ...
                client = threading.Thread(
                    target=self._handle_client_tcp, args=(conn, addr))  # !falta el handle
                client.start()
        ...
        with socket.socket(socket.AF_INET, socket.SOCK_DGRAM) as s:
            ...
            while True:
                ...
                thread = threading.Thread(
                    target=self.handle_broadcast, args=(data_recv,))  # !falta el handle
                thread.start()
    ...
    def start_udp_server(self):
        ...
        with socket.socket(socket.AF_INET, socket.SOCK_DGRAM) as s:
            ...
            while True:
                data_recv = s.recvfrom(1024)
                client = threading.Thread(
                    target=self._handle_client_udp, args=(data_recv,))  # !falta el handle
                client.start()
    # end region
    # region Protocols
    ...
    def handle_broadcast(self, data_recv: tuple):
        ...
        option = data[0]
        print(
            f'Recived data by broadcast: {data} from {addr[0] if self.ip != addr[0] else "myself"}')
        ...
        if option == JOIN:
            ...
            if addr[0] != self.ip:
                ...
                # si eres menor que yo pero soy el "first", estas entre el "lider" y yo
                    ...
                    send_data(UPDATE_SUCC, self.predecessor.ip,
                              self.port_udp, f'{ip}|{port}')
                    self.predecessor = NodeReference(addr[0], self.port_tcp)
                ...
                # si eres mayor que yo pero yo soy el "lider", entonces estas entre el "first" y yo
                    ...
                    send_data(UPDATE_PREDECESSOR, self.successor.ip,
                              self.port_udp, f'{ip}|{port}')
                    self.successor = NodeReference(addr[0], self.port_tcp)
            ...
            if addr[0] != self.ip:
                ...
                if self.successor.id == id:
                    ...
                    send_data(UPDATE_PREDECESSOR,
                              addr[0], UDP_PORT, f'{self.ip}|{self.port_tcp}')
                    ...
                    self.broadcast.update_finger(
                        id, self.ip if self.id > set_id(addr[0]) else addr[0], TCP_PORT)
    ...
    def _handle_client_udp(self, data_recv: tuple):
        ...
        if option == CONFIRM_JOIN:
            ...
            if len(data) != ['']:
                ...
                self.sucessor = NodeReference(data[3], int(data[4]))
                self.joined = True
            ...
            ip = data[1]
            port = int(data[2])
            self.sucessor = NodeReference(ip, port)
    ...
    def _handle_client_tcp(self, conn: socket.socket, addr: tuple):
        ...
        if option == REGISTER:
            ...
            if self.predecessor.id != self.successor.id:
                send_data(DATA_PRED, self.successor.ip,
                          self.port_udp, self.repli_pred)
